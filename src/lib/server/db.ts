...
const globalForDb = globalThis as unknown as { sql: postgres.Sql | undefined; initialized: boolean | undefined };

export const sql = globalForDb.sql ?? postgres(env.DATABASE_URL, { prepare: false, max: 5 });
...
export async function ensureAdminTables(): Promise<void> {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`CREATE TABLE IF NOT EXISTS admin_properties (
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      CONSTRAINT admin_properties_client_fk FOREIGN KEY (client_id) REFERENCES admin_clients(id) ON DELETE SET NULL
    )`;
    ...
    await tx`CREATE TABLE IF NOT EXISTS admin_orders (
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      CONSTRAINT admin_orders_client_fk FOREIGN KEY (client_id) REFERENCES admin_clients(id) ON DELETE SET NULL,
      CONSTRAINT admin_orders_property_fk FOREIGN KEY (property_id) REFERENCES admin_properties(id) ON DELETE SET NULL
    )`;
    ...
    await tx`CREATE TABLE IF NOT EXISTS admin_order_items (
      ...
      unit_price_cents INTEGER NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      CONSTRAINT admin_order_items_order_fk FOREIGN KEY (order_id) REFERENCES admin_orders(id) ON DELETE CASCADE,
      CONSTRAINT admin_order_items_service_fk FOREIGN KEY (service_id) REFERENCES admin_services(id) ON DELETE RESTRICT
    )`;
